...
function ui_build(job)
{
    ...
    $("#newobjectbutton").click(function() {
        ...
        {
            $("#CITIS_acceptfirst").effect("pulsate");
        }
    });
}
...
function ui_setup(job)
{
    ...
    $("#testbar").append("<div id='gobackbutton' class='button'>testHIT</div>");

    if (mturk_isoffline())
    {
        $("#submitbutton").html("保存");
        $("#gobackbutton").html("返回上一页");
    }
...
{
    var overlay = $('<div id="CITIS_overlay"></div>').appendTo("#container");
...
{
    console.dir(tracks);/*显示javascript对象的属性*/
    ...
    var note = $("<div id='submitdialog'></div>{% csrf_token %}").appendTo("#container");
...
{
    $('<div id="CITIS_overlay"></div>').appendTo("#container");
...
function ui_disable()
{
    if (ui_disabled++ == 0)
    {
        ...
        $("#gobackbutton").button("option", "disabled", true);
...
function ui_enable()
{
    if (--ui_disabled == 0)
    {
        ...
        $("#gobackbutton").button("option", "disabled", false);
