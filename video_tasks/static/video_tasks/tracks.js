...
function BoxDrawer(container)
{
    ...
    {
        var xtl = Math.min(xc, this.startx); //min x
        var ytl = Math.min(yc, this.starty); // min y
        var xbr = Math.max(xc, this.startx); //max x
        var ybr = Math.max(yc, this.starty); //max y
...
function TrackCollection(player, job)
{
    ...
    this.add = function(frame, position, color)
    {
        ...
        this.tracks.push(track);    //添加新的track
    ...
    this.draggable = function(value)
    {
        ...
        {
            this.tracks[i].draggable(value); //Track 的draggable
        }
    }
    ...
    this.resizable = function(value)
    {
        for (var i in this.tracks)
        {
            this.tracks[i].resizable(value);//Track 的resizable
        }
    }
    ...
    this.recordposition = function()
    {
        for (var i in this.tracks)
        {
            this.tracks[i].recordposition();  //Track 的 recordposition
        }
    }
    ...
    this.serialize = function()
    {
        ...
        for (var i in this.tracks)
        {
            if (!this.tracks[i].deleted)
            {
                str += this.tracks[i].serialize() + ",";  //Track序列化
                count++;
            }
        }
        if (count == 0)
    ...
    track_collection_dump = function() {
        return me.serialize();  //返回Tracks的序列化
    };
}
...
function Track(player, color, position)
{
    ...
    /*
     * Dims the visibility of the box.  使方框变暗
     */
    this.dim = function(value)
    {
        if (value)
     ...
     * attempts to interpolate or extrapolate.   插值和外推
     */
...
//        }
        ...
        return new Position(xtl, ytl, xbr, ybr, occluded, outside);  //左上角x坐标 左上角y坐标 右下角x坐标 右下角y坐标
    }
...
function Journal(start, blowradius)
{
    ...
    this.mark = function(frame, position) 
    {
        ...
        {
            if (Math.abs(i - frame) >= this.blowradius)//abs 返回绝对值
            {
