var CITIS_timeaccepted = (new Date()).getTime();
...
function mturk_submit(callback)
{
    ...
    $("body").append('<form method="get" id="CITIS_mturk">' +
        '<input type="hidden" name="assignmentId" value="">' +
        '<input type="hidden" name="data" value="" />' +
        '</form>');

    $("#CITIS_mturk input").val(params.assignmentid);
    $("#CITIS_mturk").attr("action", params.action);
        
    // function that must be called to formally complete transaction
    function redirect()
    {
        server_request("CITIS_savejobstats", 
            [params.hitid, CITIS_timeaccepted, now], 
            function() {
    ...
    {
        server_request("CITIS_markcomplete",
...
function mturk_acceptfirst()
{
    ...
    {
        var af = $('<div id="CITIS_acceptfirst"></div>').prependTo("body")
...
{
    var stc = $('<div id="CITIS_workerstats"><div id="CITIS_workerstatscontent"></div></div>');
    st = stc.children("#CITIS_workerstatscontent");
...
{
    var timer = $('<div id="CITIS_timer"></div>');
    var button = $('<input type="button" value="Start Timer" id="CITIS_timer_button">').appendTo(timer);
    var tvalue = $('<div id="CITIS_timer_value">0m 00s</div>').appendTo(timer);
...
{
    $("#CITIS_timer input").attr("disabled", "disabled");
}

function mturk_enabletimer()
{
    $("#CITIS_timer input").attr("disabled", "");
}
...
function mturk_showstatistics()
{
    ...
    server_jobstats(function(data) {
        ...
        var reward = $('<div id="CITIS_workerstatsreward"></div>');
        var amount = Math.round(data["reward"] * 100);
        var bonuses = data["bonuses"]

        var rewardstr = '<div class="CITIS_workerstatsnumber">' + amount + ' &cent;</div> pay';

        for (var i in bonuses)
        {
            rewardstr += ' + <div class="CITIS_workerstatsnumber">' + Math.round(bonuses[i][0] * 100) + ' &cent;</div> ' + bonuses[i][1];
        }
        ...
        if (!data["newuser"])
        {
            ...
            var subm = $('<div class="CITIS_workerstatsperformance"></div>');
            var submn = $('<div class="CITIS_workerstatsnumber"></div>');
            submn.html(data["numsubmitted"]);
            submn.appendTo(subm);
            subm.append(" submitted");
            subm.appendTo(st);

            var accp = $('<div class="CITIS_workerstatsperformance"></div>');
            var accpn = $('<div class="CITIS_workerstatsnumber"></div>');
            accpn.html(data["numaccepted"]);
            accpn.appendTo(accp);
            accp.append(" accepted");
            accp.appendTo(st);

            var rejt = $('<div class="CITIS_workerstatsperformance"></div>');
            var rejtn = $('<div class="CITIS_workerstatsnumber"></div>');
...
function mturk_showdonate(callback)
{
    ...
    str += "<input type='button' id='CITIS_donate_close' value='Submit HIT'>";
    str += "</p>";
    ...
    server_jobstats(function(data) {
        ...
        var overlay = $('<div id="CITIS_overlay"></div>').appendTo("body");

        var donation = $('<div id="CITIS_donation"></div>').appendTo("body");
        donation.append(str);

        $("#CITIS_donate_close").click(function() {
            var amount = $("input[name=donateamt]:checked").val();
            var params = mturk_parameters();
            server_request("CITIS_savedonationstatus", [params.hitid, amount],
                            function() {
                                overlay.remove();
                                donation.remove();
                                callback();
...
function eventlog(domain, message)
{
    ...
    CITIS_event_log.push([timestamp, domain, message]);
...
function eventlog_save(callback)
{
    if (mturk_submitallowed())
    {
        ...
        var data = "[";
        var counter = 0;
        for (var i in CITIS_event_log)
        {
            data += "[" + CITIS_event_log[i][0] + ",";
            data += "\"" + CITIS_event_log[i][1] + "\",";
            data += "\"" + CITIS_event_log[i][2] + "\"],";
            counter++;
        }
        if (counter == 0)
        {
        ...
        console.log(data);
        server_post("CITIS_saveeventlog", [params.hitid], data, function() {
            callback();
        });
    }
    else
...
function server_jobstats(callback)
{
    if (server_jobstats_data == null)
    {
        ...
        if (params.workerid)
        {
            server_request("CITIS_getjobstats",
